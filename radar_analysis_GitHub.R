...
##### PART OF SUBPLOT A FOR FIG3
non_linear_CI_simple_GAM <- ggplot(group_seq_long[group_seq_long$type=="af",], 
                                   aes(x = group_size_of_interest, y = fit)) +
  geom_line(linewidth = 2, color="firebrick") +
  coord_cartesian(
    xlim = flock_size_99_percentiles) +
  scale_y_continuous(breaks = seq(0, 90, by=10)) +
  scale_x_continuous(labels = scales::comma) +
  ylab("Directional error (°)") +
  xlab("Group size") +
...
non_linear_CI_simple_GAM
#ggsave(filename = paste0(plot_dir, "nonlinear_groupsize.png"), plot = non_linear_CI_simple_GAM, width = 5, height = 5, dpi = 300)

#####  FIG3 PROPER

img_cycle <- readPNG("Allee_effect_cycle3.png")
